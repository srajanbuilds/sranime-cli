...
- writing regexes
You'll also need web browser with a debugger and environment that can run unmodified sranime-cli

## The scraping process
The following flowchart demonstrates how sranime-cli operates from a scraping standpoint:

![image](.assets/sranime-cli-scraping-flow.png)
...
If you have done everything correctly, you can run `sranime-cli`, query your site of choice and select from the responses.
Then sranime-cli should fail without a message.
If it fails with `No results found!` you have debugging to do.
Running sranime-cli with `sh -x` is a good way to debug.
...
Then sranime-cli should fail with `Episode not released!`
...
## Other functionality
Assuming you completed all the necessary modifications, sranime-cli should completely work for you now.
...
There might be cases that can't be covered by the current structure of sranime-cli, but still it works for most sites as I've observed.

## UX Spec

There also exists a UX spec if you want to replicate the sranime-cli user experience in a fresh codebase:
![image](.assets/sranime-cli-ux-spec.png)
